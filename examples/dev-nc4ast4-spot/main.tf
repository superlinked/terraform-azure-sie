# SIE AKS Cluster — Development Example (NC4as T4 Spot)
#
# Creates an AKS cluster with one Standard_NC4as_T4_v3 spot GPU pool
# (NVIDIA T4), scale-to-zero (min=0), and up to 5 GPU nodes. T4 is the
# cheapest single-GPU SKU on Azure, with 16 GiB VRAM.
#
...
#   2. Standard NCASv3_T4 family vCPU quota in the target region (request via Azure portal)
  ...
  gpu_node_pools = [
    {
      ...
      spot       = true
      labels = {
        "sie.superlinked.com/gpu-type" = "t4"
      }
    },
  ]
